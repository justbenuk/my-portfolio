...
import { updateProjectFormSchema } from "@/validators/dashboard-validators";
...
export default function EditProjectForm({ categories, project }: EditProps) {
  ...
  async function handleForm(values: z.infer<typeof updateProjectFormSchema>) {
    // Convert tags string to array for database
    const formattedData = {
      ...values,
      tags: values.tags ? values.tags.split(',').map(tag => tag.trim()) : []
    };

    const { success, message } = await updateProjectAction(formattedData);
        ...
        <FieldGroup>
          ...
          <div className="flex gap-3 pt-4">
            ...
            >
              ...
              <span>{form.formState.isSubmitting ? "Updating..." : "Update Project"}</span>
            </Button>
            <Button
              type="button"
